...
class CapitalOne:
    ...
    async def create_uk_account(self, quantity: int = 1, num_transactions: int = 0, live_balance: bool = True) -> Dict[str, Any]:
    ...
    async def add_transactions(self, accountId):
        """ create a custom transaction using the CapitalOne API """
        url = self.build_url(f'transactions/accounts/{accountId}/create')
    ...
    async def add_fraud_transaction(self, accountId, fraudType=None):
        ...
        if fraudType is None:
            async with self.session.post(url) as response:
                return await response.json()
        data={'fraudType': fraudType}
        async with self.session.post(url, json=data) as response:
            return await response.json()
